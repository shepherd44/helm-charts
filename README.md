...
| [`cp-ksql-server`](charts/cp-ksql-server/README.md) | Confluent ksqlDB 8.3 | needs an existing Kafka; vendored from the same deleted confluent repo. Upgrading a server with persistent queries is not a tag swap — see its CHANGELOG |
